...
import { Button, Row, Col, Statistic, Divider, Card, Tag, Empty, Spin, Modal, message } from 'antd';
import { Link } from 'react-router-dom';
import { SPEAKERS_LIST, MODERATED, UNMODERATED, FORMAL, COMMENT, CRISIS } from '../constants';
      ...
      <Card 
        title={
          <Row type="flex" align="middle" justify="space-between">
            <Col span={8}>
              <Statistic title={comment.type} value={comment.score} suffix="/10"/>
            </Col>
            <Col span={5}>
              <Row type="flex" align="middle" justify="space-between">
                <Col style={{fontSize: "20px", fontWeight: "600"}}>
                  <Link to={{pathname: `/grade/${this.props.delegation}`, state: {edit: comment}}}>
                ...
                <Col style={{fontSize: "20px", fontWeight: "600"}}>
                  <Button 
                    ...
                    shape="circle" 
                    type="danger" 
                    onClick={() => this.setState({deletingComment: comment.row})}
                  />
                </Col>
              </Row>
            </Col>
  ...
  render () {
    ...
    return(
      <div>
        ...
        <Divider />
        <Row type="flex" align="bottom" style={{width: "30em", margin: "3em auto 3em 0"}}>
        ...
        <Row type="flex" align="bottom" style={{width: "30em", marginTop: "5em"}}>
          <Col span={12} style={{fontSize: "25px", fontWeight: 600}}> 
            Times Spoken
          </Col>
          <Col span={8} style={{float: "right", fontSize: "40px", marginBottom: "-7px"}}> 
            {this.state.timesSpoken}
          </Col>
        </Row>
        <Row type="flex" justify="space-around" style={{marginTop: "3em"}}>
          <Col>
            <Statistic title="Speaker's List" value={this.state.spoken[SPEAKERS_LIST]} suffix="times"/>
          </Col>
          <Col>
            <Statistic title="Moderated" value={this.state.spoken[MODERATED]} suffix="times"/>
          </Col>
          <Col>
            <Statistic title="Unmoderated" value={this.state.spoken[UNMODERATED]} suffix="times"/>
          </Col>
          <Col>
            <Statistic title="Formal" value={this.state.spoken[FORMAL]} suffix="times"/>
          </Col>
          <Col>
            <Statistic title="Comment" value={this.state.spoken[COMMENT]} suffix="times"/>
          </Col>
          <Col>
            <Statistic title="Crisis" value={this.state.spoken[CRISIS]} suffix="times"/>
          </Col>
        </Row>
        <Row span={12} style={{fontSize: "25px", fontWeight: 600, marginTop: "4em"}}>
          Comments
        </Row>
        ...
        <Modal
          ...
          footer={[
            <Button key="Not Really" onClick={() => this.setState({deletingComment: undefined})}>
              Not Really
            </Button>,
            <Button key="Yes" type="primary" onClick={() => this.deleteComment(this.state.deletingComment)}>
              Yes
            </Button>,
          ]}
        >
