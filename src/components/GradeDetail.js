...
import { Form, Button, Input, Col, Row, Affix, Divider, Tag, message } from 'antd';
...
class GradeDetail extends React.Component {
  ...
  handleKeyPress = e => {
    if (e.key === 'Enter' && e.shiftKey && !(this.state.type < 0 || this.state.score < 0)) {
      this.handleSubmit()
    }

    if (this.state.focus === 0) {
      if (!(parseInt(e.key) in [1, 2, 3, 4, 5, 6]))
        return;
            ...
            <Row type="flex">
              ...
              <Col span={18}>
                <Row type="flex" justify="center">
                  {[SPEAKERS_LIST, MODERATED, UNMODERATED, FORMAL, COMMENT, CRISIS].map((val, i) => 
                    <Col key={i} >
                      <Button 
                        size='large' 
              ...
              <Col span={18}>
                {TAGS.map((val, i) => 
                  <Tag.CheckableTag 
                    ...
                    style={{border: this.state.tags.has(val) ? null : "1px solid rgb(217, 217, 217)" }}
                  >
                    {val}
                  </Tag.CheckableTag>
                )}
